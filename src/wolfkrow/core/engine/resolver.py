...
class Resolver(object):
    ...
    def process_sg_fields(self, entity_type, fields):
        ...
        for field in fields:
            field_value = self._process_sg_field(entity_type, field, fields[field])
    ...
    def _process_sg_field(self, entity_type, field, field_value):
        ...
        # If we can't resolve the schema, then there is no processing to do
        entity_schema = self._get_schema(entity_type)
        if not entity_schema:
            print("Warning: Could not resolve schema for entity type: {}".format(entity_type))
            return field_value
